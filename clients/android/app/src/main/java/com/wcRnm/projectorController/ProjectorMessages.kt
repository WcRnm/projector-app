...
fun msgHeartbeat(handle: Int): ByteArray {
...
fun msgConnect(ipid: Int): ByteArray {
    return byteArrayOfInts(
        1,
        0,
        7,
...
fun msgEndOfQueryResponse(handle: Int): ByteArray {
    return byteArrayOfInts(
        5,
        0,
        5,
...
fun msgUpdateRequest(handle: Int): ByteArray {
    return byteArrayOfInts(
        5,
        0,
        5,
...
fun msgDigitalCommand(handle:Int, stdCmd:Int, value:Boolean, extraParam:Int) : ByteArray
{
    ...
    cmd += when (value) {
        false -> ((iCmd / 256) or 128).toByte()
        true  -> (iCmd / 256).toByte()
    }
